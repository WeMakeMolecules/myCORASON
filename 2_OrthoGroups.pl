...
sub run_blast{
	...
	`perl -I \. header2.pl`;
	my $e=shift;
	`makeblastdb -in MINI/Concatenados.faa -dbtype prot -out MINI/Database.db -logfile run`;
	`blastp -db MINI/Database.db -query MINI/Concatenados.faa -outfmt 6 -evalue $e -num_threads 20 -out $BLAST2 -logfile run`;
