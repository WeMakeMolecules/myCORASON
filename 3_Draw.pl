...
my $w=1100;  			## Size of the window
my $t=0; 			##Traslation factor horizontal
my $tv=0; 			##Translation factor Vertical
my $s=16.3; 			#Vertical Separation factor
my $h=60*($nClust); 		# 100 of heigth for draw each organisms
...
open (OUT, ">GENE_CONTEXT.svg") or die $!;
    # now render the SVG object, implicitly use svg namespace
print OUT $svg->xmlify;
close OUT;
`perl -p -i -e 's/&//' *.svg`;
`perl -p -i -e 'if(/\<polygon/){s/title=\"/\>\n\<title\>/g;if(m{\/\>\$}){s{\" \/\>}{\<\/title\>\<\/polygon\>};}}else{if((!/^\t/) and m{\/\>}){s{\" \/>}{<\/title><\/polygon>};}}' GENE_CONTEXT.svg`;
##################################################################
###    subs ######################################################
##################################################################
sub Draw{
...
##label
  ...
  my $desc="Identity ".$opacity." PEG:  ".$id_peg." Annotation: ".$func;
...
foreach my $context(@CLUSTERS){
		...
		if ($text!=0){	
			...
			my $gen=$sp[-1];
			$svg->text( x  => 10+$t, y  => $refYCOORD->[$cont_number-1]+$tv-20)->cdata("$key $orgName Gene:$gen"); 
			} ##up right;
		####################################################################
