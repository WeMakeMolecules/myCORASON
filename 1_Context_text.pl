...
sub MakeBlast{
	...
	else{ ##if Concatenados.faa does nto exists prints a warning
		if ($type eq 'nuc'){
			blastnSeq($evalueL,$file);	
...
sub blastpSeq{
	...
	`blastp -db $DBname.db -query $name.query -outfmt 6 -evalue $e -out $name.BLAST.pre`;
	open (PREBLAST,"$name.BLAST.pre") or die "Could not open $name.BLAST.pre $!";
	open (BLAST,">$name.BLAST") or die "Could not open $name.BLAST $!";
#	open (PARSER,">$name.PARSER") or die "Could not open $name.BLAST $!";  #writes the fasta file
	...
	`blastp -db $DBname.db -query $name.query -evalue $e -out $name.parser.pre` ;
...
sub setColor{
	...
	if (exists $CLUSTERcolor{$peg}[$orgs]){ ## any peg in any genome
		...
		foreach $color_percent (@{$CLUSTERcolor{$peg}[$orgs]}){ ## Puede parecerse a distintos miembros del cluster indicados por los colores, el numero de color es el numero de gen en el cluster
			...
			if($percentInHash>$percentF and $colorInHash ne ""){ #best is selected
